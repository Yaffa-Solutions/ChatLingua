...
export function createProfilePage() {
  ...
  const banner = createElement("div", ["relative", "h-10"]);
  ...
  const alertBox = createElement("span", [
    "mx-8",
    "hidden",
    "block",
    "rounded-xl",
  ...
  const imageUrlLabel = createElement(
    "label",
    ["block", "mb-1.5", "text-sm", "font-medium", "text-gray-800"],
    "Image URL"
  );
  ...
  const refreshSummary = () => {
    summary.innerHTML = "";
    const n =
      Number(nativeSelect.value) > 0
        ? `Native: ${LANGS[Number(nativeSelect.value)]}`
        : "";
    const l =
      Number(learningSelect.value) > 0
        ? `Learning: ${LANGS[Number(learningSelect.value)]}`
        : "";
  ...
  imageUrlInput.addEventListener("input", () =>
    clearFieldError(imageUrlDiv, imageUrlInput)
  );
  ...
  saveBtn.addEventListener("click", async (e) => {
    ...
    if (!nativeIdx) {
      setFieldError(nativeDiv, nativeSelect);
      errors.push("Please select your native language.");
    }
    if (!learningIdx) {
      setFieldError(learningDiv, learningSelect);
      errors.push("Please select the language you are learning.");
    }
    if (nativeIdx && learningIdx && nativeIdx === learningIdx) {
      setFieldError(learningDiv, learningSelect);
      errors.push("Learning language must be different from native.");
    }
    if (!imageUrl) {
      setFieldError(imageUrlDiv, imageUrlInput);
