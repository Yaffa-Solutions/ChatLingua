...
const deleteMessageFor=(req,res,next)=>{
  ...
  .then(({rows})=>{
    res.status(200).json({message:`Message deleted for this profile :${profile_id}`,data:rows})
  }).catch(err=>next(err))
}
...
const checkIsDeletedChat=(req,res,next)=>{
  ...
  if(!chat_id || !profile_id){
    return res.status(400).json({message:'Invalid Data For this  chat_id , profile_id'});
  }
