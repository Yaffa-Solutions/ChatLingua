    ...
    >
      <div className={`flex gap-2 max-w-[75%] ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
        <div className="flex-shrink-0 self-end mb-1">
          <div className="w-6 h-6 rounded-full overflow-hidden ring-2 ring-white/30">
            {isOwn ? (
              user?.image ? (
                <img src={user.image} alt="" className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full bg-marigold flex items-center justify-center text-ink text-xs font-bold">
                  {(user?.username || 'U')[0]}
                </div>
              )
            ) : (
              message.sender_image ? (
                <img src={message.sender_image} alt="" className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full bg-marigold-tint flex items-center justify-center text-marigold-deep text-xs font-bold">
        ...
        <div className="relative group">
          ...
          >
            ...
            {time && (
              <p className={`text-[11px] mt-1.5 opacity-50 ${isOwn ? 'text-paper/60' : 'text-ink-40'}`}>
                {formatTime(time)}
              </p>
            )}
          </motion.div>
          ...
          <div className={`absolute top-0 ${isOwn ? 'left-0 -translate-x-full pl-2' : 'right-0 translate-x-full pr-2'} opacity-0 group-hover:opacity-100 transition-opacity duration-150`}>
            <div className="flex flex-col gap-0.5">
              <button
                ...
                className="w-7 h-7 flex items-center justify-center rounded-full bg-white border border-stone-line text-ink-40 hover:text-marigold hover:border-marigold/30 transition-colors shadow-sm"
                title="Translate"
              >
              ...
              <div className="relative">
                <button
                  ...
                  className="w-7 h-7 flex items-center justify-center rounded-full bg-white border border-stone-line text-ink-40 hover:text-redline hover:border-redline/30 transition-colors shadow-sm"
                  title="More"
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {showMenu && (
                  <motion.div
                    ...
                    className={`absolute ${isOwn ? 'left-0' : 'right-0'} top-8 w-32 bg-white rounded-sm border border-stone-line shadow-sm overflow-hidden z-20`}
                  >
                    <button
