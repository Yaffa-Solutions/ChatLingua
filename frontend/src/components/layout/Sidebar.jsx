          ...
          <>
            ...
            >
              <div className="flex-1 bg-white border-r border-stone-line m-2 lg:m-0 lg:rounded-none flex flex-col overflow-hidden shadow-sm">
                {/* Profile header */}
                <div className="p-3 border-b border-stone-line">
                  <div className="flex items-center gap-2.5">
                    <Avatar src={user?.image} alt={user?.username || 'User'} size="md" status="online" />
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-ink text-[14px] truncate">
                        {user?.username || 'User'}
                      </h3>
                    ...
                    <motion.button
                      ...
                      className="w-7 h-7 flex items-center justify-center rounded-sm bg-white border border-stone-line text-ink-40 hover:text-ink hover:bg-paper/50 transition-all flex-shrink-0"
                    ...
                    profiles
                      ...
                      .map((profile) => (
                        <motion.button
                          ...
                          className={`
                            w-full flex items-center gap-2.5 px-3 py-2 rounded-sm transition-all duration-150 text-left
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-ink text-[14px] truncate">{profile.username}</p>
                            <p className="text-[12px] text-ink-70 truncate">
                              Native in {langName(profile.native_language_id)}
                            </p>
                          </div>
                          <div className="w-1.5 h-1.5 rounded-full bg-sprout flex-shrink-0" />
                        </motion.button>
                      ))
                  )}
                </div>
